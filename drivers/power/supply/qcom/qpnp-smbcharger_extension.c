...
#define UNPLUG_WAKE_PERIOD		(3000) /* three seconds */
void somc_unplug_wakelock(struct chg_somc_params *params)
{
	__pm_wakeup_event(&params->wakeup_src, UNPLUG_WAKE_PERIOD);
}
...
static int somc_chg_register(struct smbchg_chip *chip)
{
	...
	wakeup_source_init(&params->wakeup_src, "unplug_wakelock");
...
static void somc_chg_unregister(struct smbchg_chip *chip)
{
	...
	wakeup_source_trash(&params->wakeup_src);
	if (params->apsd.wq)
