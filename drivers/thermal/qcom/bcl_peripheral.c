...
#define BCL_VBAT_MIN_LEGACY   0x58
#define BCL_IBAT_MAX_LEGACY   0x59
...
#define BCL_VBAT_MIN_CLEAR    0x66
#define BCL_IBAT_MAX_CLEAR    0x67
...
static int bcl_clear_vbat_min(void)
{
	...
	if (is_legacy_bcl)
		ret = bcl_write_register(BCL_VBAT_MIN_CLEAR, BIT(7));
	else
...
static int bcl_clear_ibat_max(void)
{
	...
	if (is_legacy_bcl)
		ret = bcl_write_register(BCL_IBAT_MAX_CLEAR, BIT(7));
	else
		ret = bcl_write_register(BCL_MAX_MIN_CLR,
...
static int bcl_read_ibat(void *data, int *adc_value)
{
	...
	*adc_value = (int)val[VAL_REG_BUF_OFFSET];
	do {
		if (is_legacy_bcl)
			ret = bcl_read_multi_register(BCL_IBAT_MAX_LEGACY, val,
				VAL_CP_REG_BUF_LEN);
		else
...
static int bcl_read_vbat(void *data, int *adc_value)
{
	...
	*adc_value = (int)val[VAL_REG_BUF_OFFSET];
	do {
		if (is_legacy_bcl)
			ret = bcl_read_multi_register(BCL_VBAT_MIN_LEGACY, val,
				VAL_CP_REG_BUF_LEN);
		else
...
static int bcl_probe(struct platform_device *pdev)
{
	...
	is_legacy_bcl = false;
	if (of_device_is_compatible(pdev->dev.of_node, "qcom,msm-bcl"))
		is_legacy_bcl = true;
